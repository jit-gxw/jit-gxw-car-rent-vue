...
import {appointmentPageService,appointmentRejectionService,appointmentPassService,appointmentDeleteService,appointmentStatusNumberService} from '@/api/appointment'
...
const getStatusNumber=async ()=>{
  await appointmentStatusNumberService()
  .then((result) => {
...
const pageList=async ()=>{
    await appointmentPageService(pageQuery.value)
    .then((result) => {
...
const rejectionHandle=async (formEl)=>{
  ...
  await formEl.validate(async (valid) =>{
    if(valid){
      if (confirm('确认驳回预约?', '提示')){
        await appointmentRejectionService(rejectDate.value)
        .then((result) => {
...
const passHandle=async ()=>{
  if (confirm('确认通过预约?', '提示')){
    await appointmentPassService({id:detailDataModel.value.id})
    .then((result) => {
      ElMessage.success('提交成功！')
      pageList()
      dialogCheckVisible.value=false
      confirm('生成订单：'+result.data.number, '提示');
    }).catch((err) => {
...
const deleteHandle=async (id)=>{
  if (confirm('确认删除该预约?', '提示')){
    await appointmentDeleteService({id:id})
    .then((result) => {
...
<template>
        ...
        <template #header>
            <div class="header">
                ...
                <label style="margin-right: 0px;margin-left: 0px">用户姓名：</label>
                <el-input
                v-model="userNameInput"
                class="w-50 m-2"
                placeholder="搜索预约用户姓名"
                :prefix-icon="Search"
                style="width: 10%;margin-right: 0px;margin-left: 0px"
                />
                ...
                <label style="margin-right: 0px;margin-left: 0px">预约时间：</label>
                <el-date-picker
                    v-model="beginTime"
                    type="date"
        ...
        <el-table :data="appointmentData" style="width: 100%">
            ...
            <el-table-column label="用户" prop="userName" width="110"></el-table-column>

            <el-table-column label="车牌号码" prop="licensePlateNumber" width="110">
            ...
            <el-table-column label="取车时间" prop="collectionTime" width="180"></el-table-column>

            <el-table-column label="还车时间" prop="returnTime" width="180"></el-table-column>

            <el-table-column label="发起时间" prop="appointmentTime" width="180"></el-table-column>
            ...
            <el-table-column label="当前预约状态" prop="status" width="120">
                <template v-slot="scope">
        ...
        <!-- 描述信息 -->
         <el-descriptions
          ...
          <el-descriptions-item min-width="150px">
            ...
            {{detailDataModel.appointmentInfo}}
          </el-descriptions-item>
          ...
          <el-descriptions-item min-width="150px">
            ...
            {{detailDataModel.appointmentTime}}
          </el-descriptions-item>
    ...
    <el-dialog v-model="dialogCheckVisible" title="审核预约" width="50%">
        ...
        <el-descriptions
         ...
         >
           ...
           <el-descriptions-item label="预约用户姓名：" min-width="100px">{{detailDataModel.userName}}</el-descriptions-item>

           <el-descriptions-item label="用户身份证号：" >{{detailDataModel. idNumber}}</el-descriptions-item>

           <el-descriptions-item label="用户驾驶证号：" >{{detailDataModel.licenceId}}</el-descriptions-item>

           <el-descriptions-item label="用户手机号：" >{{detailDataModel.phone}}</el-descriptions-item>
           
           <el-descriptions-item label="预约车辆名称：" >{{detailDataModel.vehicleName}}</el-descriptions-item>

           <el-descriptions-item label="车牌号码：" >{{detailDataModel.licensePlateNumber}}</el-descriptions-item>

           <el-descriptions-item label="预约用车时间：" >{{detailDataModel.collectionTime}}至{{ detailDataModel.returnTime }}</el-descriptions-item>

           <el-descriptions-item label="预约信息：" >{{detailDataModel.appointmentInfo}}</el-descriptions-item>
